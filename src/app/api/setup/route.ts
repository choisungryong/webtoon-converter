...
const ALTER_STATEMENTS = [
  `ALTER TABLE generated_images ADD COLUMN original_r2_key TEXT`,
  `ALTER TABLE generated_images ADD COLUMN type TEXT DEFAULT 'generated'`,
  `ALTER TABLE generated_images ADD COLUMN source_image_ids TEXT`,
];
  ...
  try {
    ...
    for (const sql of TABLE_STATEMENTS) {
      try {
        ...
        results.push(`SKIP: ${sql.slice(0, 50)}... (${(e as Error).message})`);
      }
    }
    ...
    for (const sql of ALTER_STATEMENTS) {
      try {
        ...
        results.push(`SKIP: ${sql} (already exists)`);
      }
    }
